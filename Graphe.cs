...
namespace Karaté
{
    internal class Graphe
    {
        ...
        public Graphe(string fichier)
        {
            ...
            ParcoursLargeur(noeuds, noeuds[0].Numero);

            Console.Write("Le graphe ");
            if (ContientCycle(noeuds)) { Console.Write("contient un ou plusieurs cycle "); }
            else { Console.Write("ne contient pas de cyle "); }
            if (EstConnexe(noeuds)) { Console.WriteLine("et est connexe."); }
            else { Console.WriteLine("et n'est pas connexe."); }
            

            VisualiserGraphe(noeuds);
        }
        static bool ContientCycle(Noeud[] graphe)
        {
        ...
        static bool EstConnexe(Noeud[] graphe)
        {
            ...
            if (graphe.Length == ParcoursLargeur(graphe, graphe[0].Numero).Length)
            {
                result = true;
            }
            return result;
        ...
        static int[] ParcoursLargeur(Noeud[] noeud, int num)
        {
            ...
            {
                for (int j = 0; j < noeud[parcours[i]-1].Lien.Length; j++)
                {   //Si le sommet est deja présent dans la liste, alors on ne l'ajoute pas dans la liste
                    if (!parcours.Contains(noeud[parcours[i]-1].Lien[j]))
                    {
                        parcours.Add(noeud[parcours[i]-1].Lien[j]);
                    }
                }
            }
